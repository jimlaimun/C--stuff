...
void BouncingBallMain::SetupBackgroundBuffer()
{
	...
	for ( int iX = 0 ; iX < GetScreenWidth() ; iX++ )
		for ( int iY = 0 ; iY < this->GetScreenHeight()-100 ; iY++ )
			switch( rand()%100 )
			{
...
int BouncingBallMain::InitialiseObjects()
{
	...
	int posX = (0+ (rand() % (int)(GetScreenWidth() - 0 + 1)));
	m_pBall1->SetMovement( GetTime(), GetTime()+500, GetTime(), posX, 10, 560, GetScreenHeight()-50 );
	...
	m_ppDisplayableObjects[1] = m_pBall1;
	m_ppDisplayableObjects[2] = m_pBall2;
